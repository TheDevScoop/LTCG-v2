...
const TRACK_FREQUENCY_HZ: Record<string, number> = {
  summon: 440,
  spell: 520,
  attack: 620,
  turn: 740,
...
function buildToneBuffer(frequency: number): Buffer {
  ...
  for (let i = 0; i < sampleCount; i += 1) {
    ...
    const sample = Math.round(Math.sin(2 * Math.PI * frequency * t) * amplitude);
